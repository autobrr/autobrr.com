...
id: tips
sidebar_label: Tips
title: Tips for running autobrr
description: "Practical autobrr tips: stop grabs when disk space runs low, enable and download logs, and troubleshoot filters with the autobrr.log file."
...
```bash
#!/bin/bash
# autobrr checks the script's exit code, not stdout - use exec so ssh's status is returned
exec ssh user@domain '~/freespace.sh' 2>/dev/null
```
