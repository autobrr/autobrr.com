...
slug: /filters
sidebar_label: Intro
title: "Filters: how autobrr decides what to grab"
description: Filters are the conditions every announce is checked against. Learn how filters, actions and indexers fit together before building your first one.
...
The filter form is split into tabs, each with its own page: [General](./general.mdx) for indexers, priority and size limits, [TV & Movies](./tv-movies.mdx) and [Music](./music.mdx) for media-specific fields, [Advanced](./advanced.mdx) for regex, [External](./external.mdx) for scripts and webhooks, and [Actions](./actions.mdx) for what happens on a match. If you would rather start from something that works, copy one of the [filter examples](./examples.mdx).
