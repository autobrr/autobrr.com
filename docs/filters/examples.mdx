---
title: Filter examples for Sonarr, Radarr and racing
description: Ready-made autobrr filter examples for Sonarr and Radarr, plus filters for building buffer and ratio on private trackers.
keywords:
  [
    autobrr,
    filters,
...
Don't forget to add a [Sonarr action](../filters/actions.mdx#radarr-sonarr-lidarr-readarr-and-whisparr)!
...
| Match Categories | [Depends on Indexer](./categories.mdx) |
...
Don't forget to add a [Radarr action](./actions.mdx#radarr-sonarr-lidarr-readarr-and-whisparr)!
...
The **Movies / Shows** field on the [TV & Movies](./tv-movies.mdx) tab matches against the parsed title of the release, and supports the usual wildcards: `*` for zero or more characters, `?` for exactly one. Matching is case-insensitive and must cover the whole title, so add `*` when you only know part of it.
...
This means a `Shows` value of `Formula 1` matches every F1 release: practice, qualifying and race alike. To narrow down to specific sessions, use **Match releases** on the [Advanced](./advanced.mdx#releases) tab, which matches against the whole release name. It matches substrings even without wildcards, and a comma-separated list works as OR:
...
Check your indexer or our [list of indexers supporting freeleech](./freeleech.mdx) filtering for specifics.
