...
title: "External filters: scripts and webhooks"
description: External filters run a script or call a webhook before autobrr pushes a release, and only continue when the expected exit or HTTP status matches.
...
Many of the fields have support for [macros](./macros.mdx), which allow you to enhance your workflow significantly by providing custom logic/data processing to the input data provided by autobrr.
...
See [stop if disk is full](../usage/tips.mdx#stop-if-disk-is-full) for a good example of what it can do.
...
A real-world example is [cross-seeding with qui](./cross-seed-qui.mdx), where the API's status code decides between cross-seeding now, retrying later, or rejecting:
