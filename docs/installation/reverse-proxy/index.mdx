...
Running autobrr behind a reverse proxy is recommended when you want to reach the web UI from outside the box it runs on. The proxy gives you TLS, a proper domain instead of `ip:port`, and more robust authentication options on top of autobrr's own [login and OIDC](../../configuration/authentication.mdx).

Pick the guide for the proxy you run:

- [Caddy](./caddy.mdx); the simplest option, with automatic TLS
- [lighttpd](./lighttpd.mdx)
- [nginx](./nginx.mdx)
- [SWAG](./swag.mdx); nginx preconfigured for Docker setups
- [Traefik](./traefik.mdx); common in Docker and Kubernetes stacks
- [Tailscale Serve](./tailscale-serve.mdx); private access over your tailnet, no ports exposed
