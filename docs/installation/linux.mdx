...
description: Install autobrr on a Linux server manually with a systemd service, or through seedbox solutions like Swizzin, Saltbox and QuickBox.
keywords:
  [
    autobrr,
    linux,
...
Alternatively, see the installation instructions for [Docker <FaDocker />](./docker.mdx) and [Windows <FaWindows />](./windows.mdx).
...
:::info
If you do not have root, or are on a shared system, place the binaries somewhere in your home directory like `~/.bin` or use our installers for [shared seedboxes](seedbox.mdx).
:::
