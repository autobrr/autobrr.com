...
id: macos
sidebar_label: MacOS
title: Install autobrr on macOS
description: Install autobrr on macOS with Homebrew and run it as a background service under a dedicated user, so no terminal window has to stay open.
