...
title: Add indexers and trackers to autobrr
description: "How to add a tracker to autobrr: pick it from the supported indexer list, enter your passkey or credentials, and connect its IRC announce channel."
keywords:
  [
    autobrr,
    setup,
...
The list below of supported indexers are for indexers who have IRC announces. If your indexer is not in the list you can still use it via **Generic RSS** and **Generic Torznab/Newznab** (via Prowlarr or Jackett). See [Feeds](./feeds.mdx).
...
Navigate to `Settings > Indexers` to add new indexers.
   ...
   :::info
   - If NickServ Password is marked with a `*`, a registered IRC account is required. Refer to [registering with NickServ](irc.mdx#registering-with-nickserv) for more details.
...
:::caution[Important]
Refer to the [IRC](../configuration/irc.mdx) section for detailed information about NickServ, IRC keys, and nick grouping.
:::
