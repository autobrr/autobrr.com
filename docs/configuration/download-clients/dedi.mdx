...
title: Set up download clients in autobrr
slug: /configuration/download-clients/dedicated
description: Add qBittorrent, Deluge, rTorrent, Transmission, Porla, aria2, SABnzbd, NZBGet, Sonarr, Radarr, Lidarr, Readarr and Whisparr as clients in autobrr.
keywords:
  [
    autobrr,
    config,
...
These are the available download clients. On a shared seedbox, see the [shared seedbox page](./shared.mdx) for provider-specific host, port and auth details.
...
If you are on a [shared seedbox](./shared.mdx) you might need your username in the url like:
