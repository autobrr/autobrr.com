...
title: IRC networks and announce channels
description: "Set up IRC in autobrr: register a nick with NickServ, add a network, join announce channels, use SASL or a ZNC bouncer, and troubleshoot connections."
keywords:
  [
    autobrr,
    setup,
...
If the network does not accept connections from your server at all, you can route the IRC connection through a SOCKS5 or HTTP [proxy](./proxy.mdx).

You shouldn't need to worry about it, but it's nice now to know what you need to do if that were to happen.

## Setup {/* #setup */}

The initial setup of IRC networks and channels are done during the setup of [indexers](../configuration/indexers.mdx).
...
### Using Soju {/* #using-soju */}

After adding a indexer you need to change the IRC configuration:

#### Network Settings {/* #soju-network-settings */}

- Keep `Server` and `Port` as the upstream IRC server for the network you want to connect to.
- Leave `Password` (Network password) empty
- Turn Bouncer on, and set `Bouncer address` to the `HOST:PORT` of the Soju instance.

#### Identification Settings {/* #soju-identification-settings */}

- Set `Mechanism` to SASL (plain)
- Set `Account` to `<soju-username>/<network-name>@<client-name>` (https://codeberg.org/emersion/soju/src/branch/master/doc/getting-started.md#other-clients)
- Use the `Password` for your downstream soju user

Note that autobrr does not yet support IRCv3 websockets, you will need to configure soju to listen over TCP
