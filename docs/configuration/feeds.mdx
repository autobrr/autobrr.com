...
title: RSS, Torznab and Newznab feeds
description: Add RSS, Torznab and Newznab feeds as indexers in autobrr for trackers without an IRC announce channel, including polling interval and filter setup.
keywords:
...
Torznab feeds carry the `downloadvolumefactor` attribute, so the [Freeleech and Freeleech Percent](../filters/advanced.mdx) filter fields work with Torznab feeds (e.g. via Prowlarr), even for indexers whose IRC announces have no freeleech info.
