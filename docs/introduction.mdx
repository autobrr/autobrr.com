...
title: Introduction
description: "autobrr is a modern download automation tool and autodl-irssi replacement: it watches IRC announces and feeds, filters releases and pushes them to your client."
keywords:
  [
    autobrr,
    indexers,
...
New here? The [quick start](./quick-start.mdx) walks through a first setup end to end, and the [installation guide](./installation/intro.mdx) covers every platform.
...
Many indexers announce new torrents on their [IRC](./configuration/irc.mdx) channels the second it is uploaded to the site. autobrr monitors such channels in real time and grabs the torrent file as soon as it's uploaded based on certain conditions (hereby referred to as [filters](filters/intro.mdx)) that you set up within autobrr. It then sends that torrent file to a download client of your choice via an [action](./filters/actions.mdx) set within the filter. A download client can be anything from qBittorrent and Deluge, to Radarr and Sonarr, or a watch folder.
...
1. autobrr monitors IRC channels and/or RSS feeds for new torrents that fits your criteria set within your autobrr [filters](filters/intro.mdx).
2. A successful match is forwarded to your [download client](./configuration/download-clients/dedicated) of choice via an [action](./filters/actions.mdx) set inside your filters.
...
- RSS and Torznab/Newznab [feed support](./configuration/feeds.mdx) via Prowlarr to easily get access to hundreds of indexers
...
- Database engine supporting both [PostgreSQL](./installation/supplementary/postgresql.mdx) and SQLite
- [Notifications](./configuration/notifications.mdx) (Discord, Notifiarr, Telegram, Pushover, Gotify, ntfy, LunaSea, Shoutrrr, webhooks)
...
- Base path / Subfolder (and subdomain) support for convenient [reverse-proxy](./installation/reverse-proxy/index.mdx) support
...
The development of autobrr started in Early 2020, entering rapid development in Summer 2021 due to dissatisfaction with needing 3+ tools to do one job. Autobrr has since gained quite a bit of traction and has a growing [community](./community.mdx) supporting the project.
