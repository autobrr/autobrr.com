...
title: autobrr quick start
description: "Get autobrr running end to end: add an indexer and its IRC network, build a filter, connect Sonarr as the action and verify the first push."
...
other use cases may require more RTFM'ing in [the configuration docs](./configuration/autobrr.mdx).

1. [Install autobrr](./installation/intro.mdx).
...
4. [Group a "bot" nick with your real nick](./configuration/irc.mdx#grouping-nicks).

   This is the common case, but check your tracker's IRC documentation and adjust as
   appropriate.

5. Add an [indexer](./configuration/indexers.mdx).
...
7. Add a [filter](filters/intro.mdx).
   ...
   **NOTE**: Autobrr does nothing with received IRC announcements without at least one
   filter applied to at least one indexer. The [filter examples](./filters/examples.mdx) show
   some good starting points for Sonarr and Radarr.

8. Add a [filter action](./filters/actions.mdx).
...
From here you can set up [notifications](./configuration/notifications.mdx) to get a message on every push, and check the [FAQ](./faqs.mdx) if releases are not showing up.
