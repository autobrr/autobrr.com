...
| `{{.Cookie}}`                    | string   | Raw indexer cookie (cookie-auth indexers). Contains credentials, handle with care |
...
| `{{.CurrentTimeUnixMS}}`         | int      | Current unix time in milliseconds          |
...
| `{{.ReleaseTags}}`               | string   | Raw release tags string from the announce  |
